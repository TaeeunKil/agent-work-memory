...
DATABASE_FILE_NAME = "workalmanac.db"


class WorkAlmanacConfig(WorkAlmanacModel):
    state_dir: Path
...
def default_state_dir() -> Path:
    configured = os.environ.get("WORKALMANAC_STATE_DIR")
    if configured:
        return Path(configured).expanduser().resolve()
    local_app_data = os.environ.get("LOCALAPPDATA")
    if os.name == "nt" and local_app_data:
        return (Path(local_app_data) / "WorkAlmanac").resolve()
    xdg_state_home = os.environ.get("XDG_STATE_HOME")
    if xdg_state_home:
        return (Path(xdg_state_home) / "workalmanac").expanduser().resolve()
    return (Path.home() / ".local" / "state" / "workalmanac").resolve()


def load_config(state_dir: Path | None = None) -> WorkAlmanacConfig:
    ...
    return WorkAlmanacConfig(state_dir=resolved_state, vault_path=vault_path)


def save_config(config: WorkAlmanacConfig) -> None:
