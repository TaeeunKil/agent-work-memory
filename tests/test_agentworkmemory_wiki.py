...
def test_init_builds_obsidian_home_and_category_indexes(
    ...
    assert "agentworkmemory_managed: true" in home
...
""",
    ...
    assert "[[projects/agent-work-memory|Agent Work Memory project]]" in decision_index
    assert (
...
def test_windows_curator_workspace_acl_is_repaired_without_a_console(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    captured = []
    ...
    monkeypatch.setenv("USERNAME", "almanac-user")
